...
import { apiCreateBeat, apiDeleteBeat, apiFetchBeats, apiUpdateBeat } from '../lib/beat-api';
import { BEAT_TYPES, BEAT_TYPE_COLORS, BEAT_TYPE_LABELS } from '../types/beat';
import type { BeatType, StoryBeat } from '../types/beat';
          ...
          {BEAT_TYPES.map((t) => (
            <button
              ...
              style={{
                ...
                fontSize: 12,
                fontWeight: 600,
                border: `1.5px solid ${beatType === t ? BEAT_TYPE_COLORS[t] : 'var(--sw-line-default)'}`,
                background: beatType === t ? BEAT_TYPE_COLORS[t] : 'var(--sw-bg-subtle)',
                color: beatType === t ? 'white' : 'var(--sw-text-secondary)',
                cursor: 'pointer',
              }}
            >
              {BEAT_TYPE_LABELS[t]}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              cursor: 'pointer',
              opacity: !title.trim() || saveMutation.isPending ? 0.5 : 1,
              display: 'flex',
              alignItems: 'center',
              gap: 6,
            }}
...
export function StoryBeatList({ novelId }: StoryBeatListProps) {
  ...
  const nextOrderKey =
    beats && beats.length > 0 ? Math.max(...beats.map((b) => b.order_key)) + 1.0 : 1.0;
              ...
              >
                <span
                  style={{
                    ...
                    padding: '0 8px',
                    borderRadius: 6,
                    background: BEAT_TYPE_COLORS[beat.beat_type],
                    color: 'white',
                    fontSize: 11,
                    fontWeight: 700,
                    flexShrink: 0,
