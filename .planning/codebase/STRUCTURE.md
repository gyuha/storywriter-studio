...
- `web/src/main.tsx`: React DOM mount, RouterProvider
- `web/src/routes/__root.tsx`: Root layout, AppProviders, Toaster, Modals

**Configuration:**
- `apps/api/src/core/config.py`: All backend settings (DB, Redis, JWT, LLM, OAuth, mail)
- `web/vite.config.ts`: Vite plugins (TanStack Router, React, Tailwind, tsconfig-paths)
- `web/tsconfig.json`: TypeScript config with `@/*` → `./src/*` alias
...
**Frontend Auth:**
- `web/src/features/auth/store/auth.store.ts`: Zustand auth state (`useAuthStore`)
- `web/src/features/auth/hooks/use-auth-mutation.ts`: React Query mutations for login/signup
- `web/src/features/auth/lib/mock-auth-api.ts`: Mock API functions (replace with real HTTP client)

**Global State:**
- `web/src/stores/modal-store.ts`: Zustand modal stack (`useModal`)
- `web/src/providers/app-providers.tsx`: `QueryClient` singleton + `QueryClientProvider`
...
- Create `web/src/features/{feature}/` with: `components/`, `hooks/`, `store/`, `types/`, `lib/`
- Add route file: `web/src/routes/{feature}/index.tsx` using `createFileRoute`
- Route tree regenerates automatically on next Vite dev server start

**New Frontend Page (Route):**
- Add `web/src/routes/{path}.tsx` using `createFileRoute('/{path}')({ component: ... })`
- `routeTree.gen.ts` is regenerated automatically — do not edit it

**Shared UI Components:**
- Generic primitives: `web/src/components/ui/{name}.tsx`
- Layout wrappers: `web/src/components/layout/{name}.tsx`
- Shared hooks: `web/src/hooks/use-{name}.ts`

**Global State:**
- App-wide Zustand stores: `web/src/stores/{name}-store.ts`
- Feature-scoped state stays inside `web/src/features/{feature}/store/`
